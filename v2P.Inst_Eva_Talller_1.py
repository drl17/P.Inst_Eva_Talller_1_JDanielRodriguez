#V2 ---> Intento generar una segunda tabla para permitir crear múltiples cuentas para un mismo usuario, validando la existencia de la cuenta por medio de la contraseña
#Este código no está finalizado
...
class CajeroAutomatico:
    def __init__(self, nombreproyecto):
        ...
        self.c.execute('''CREATE TABLE IF NOT EXISTS cuentas (
                            id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    def crearcuenta(self):
        ...
        # Verificar si la cuenta ya existe en la base de datos consultando la contraseña
        
        self.c.execute("SELECT * FROM usuarios WHERE password = %s", (password,))
        if self.c.fetchone():
            print("Esta cuenta ya existe para este usuario, por favor cree una cuenta con una contraseña diferente.")
            return

        # Insertar los datos ingresados en la tabla de usuarios
    ...
    def depositar(self):
        ...
        if usuario:
            saldo_actual = float(usuario[3])
            nuevo_saldo = saldo_actual + cantidad
            # Actualizar el saldo del usuario en la base de datos
            self.c.execute("UPDATE cuentas SET saldo = %s WHERE usuario_id = (SELECT id FROM usuarios WHERE username = %s AND password = %s)", (nuevo_saldo, username, password))
            self.conn.commit()
    ...
    def mostrar_saldo(self):
        ...
        password = input("Ingrese su contraseña: ")
        # Verificar si la cuenta existe en la base de datos
        self.c.execute("SELECT saldo FROM cuentas WHERE usuario_id = (SELECT id FROM usuarios WHERE username = %s AND password = %s)", (username, password))
